...
function ApproveBtn({ id, actionLoading, onClick }) {
  const isActive = actionLoading?.startsWith(id);
  const label =
    actionLoading === id+'_verify' ? '🔍 Verifying...'    :
    actionLoading === id+'_saving' ? '💾 Saving...'        :
    actionLoading === id+'_qr'     ? '🎟️ Generating QR...' :
    actionLoading === id+'_email'  ? '📧 Sending Email...' :
    actionLoading === id+'_done'   ? '✅ Done!'             :
    '✅ Approve';
  const bg =
    actionLoading === id+'_done'   ? 'linear-gradient(135deg,#16a34a,#15803d)' :
    actionLoading === id+'_email'  ? 'linear-gradient(135deg,#7c3aed,#2563eb)' :
    actionLoading === id+'_qr'     ? 'linear-gradient(135deg,#0891b2,#0e7490)' :
    actionLoading === id+'_saving' ? 'linear-gradient(135deg,#b45309,#92400e)' :
    actionLoading === id+'_verify' ? 'linear-gradient(135deg,#6d28d9,#4c1d95)' :
    'linear-gradient(135deg,#16a34a,#15803d)';
  return (
    <motion.button onClick={onClick} disabled={isActive}
      className="w-full py-2 rounded-lg text-xs font-bold text-white whitespace-nowrap"
      style={{ background: bg, minWidth: 145 }}
      animate={isActive && actionLoading !== id+'_done' ? { opacity:[1,0.6,1] } : { opacity:1 }}
...
export default function AdminPage() {
  ...
  const handleApprove = async (id) => {
    for (const s of [id+'_verify', id+'_saving', id+'_qr', id+'_email']) {
      setActionLoading(s);
  ...
  const handleReject = async (id) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) { alert('❌ Rejected.'); setShowReject(false); setRejectReason(''); fetchData(tab); setSsModal(null); }
      else alert('Error: ' + json.message);
    } catch { alert('Server error.'); }
    finally { setActionLoading(''); }
  };
  ...
  const handleResetCheckin = async (id) => {
    if (!window.confirm('Reset check-in?')) return;
    try {
  ...
  const downloadExcel = () => {
    const rows = data.map((r,i) => ({
      ...
      'Unique ID':r.uniqueId||'—','Status':r.status,
        ...
        <motion.div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6"
          ...
          <div className="flex gap-2 flex-wrap">
            <motion.a href="/staff" target="_blank"
              className="px-4 py-2.5 rounded-xl font-bold text-sm text-cyan-300"
              style={{ background:'rgba(34,211,238,0.1)', border:'1px solid rgba(34,211,238,0.3)' }}
              whileHover={{ scale:1.04 }}>👥 Staff Panel</motion.a>
            <motion.button onClick={downloadExcel}
              className="px-4 py-2.5 rounded-xl font-bold text-white text-sm"
              style={{ background:'linear-gradient(135deg,#16a34a,#15803d)' }}
              whileHover={{ scale:1.04 }} whileTap={{ scale:0.96 }}>📥 Export Excel</motion.button>
            <motion.button onClick={() => fetchData(tab)}
              className="px-4 py-2.5 rounded-xl font-bold text-sm text-purple-300"
              style={{ background:'rgba(139,92,246,0.1)', border:'1px solid rgba(139,92,246,0.3)' }}
              whileHover={{ scale:1.04 }} whileTap={{ scale:0.96 }}>🔄 Refresh</motion.button>
          </div>
        </motion.div>
                      ...
                      <td className="px-3 py-3 whitespace-nowrap">
                        {r.checkedIn ? (
                          <div className="flex flex-col gap-1">
                            <span className="px-2 py-1 rounded-full text-xs font-bold bg-green-500/20 text-green-400 border border-green-500/30">✅ Checked In</span>
                            ...
                            <button onClick={() => handleResetCheckin(r._id)} className="text-[10px] text-red-400/60 hover:text-red-400 transition-colors text-left">↩ Reset</button>
                          </div>
                        ) : <span className="text-gray-500 text-xs">Not yet</span>}
                      </td>
                      <td className="px-3 py-3">
                        <button onClick={() => fetchSS(r._id)}
                          className="px-3 py-1.5 rounded-lg text-xs font-bold text-cyan-300 hover:text-white transition-colors"
                      ...
                      <td className="px-3 py-3 whitespace-nowrap">
                        {r.status==='pending' ? (
                          <div className="flex flex-col gap-1.5">
                            <ApproveBtn id={r._id} actionLoading={actionLoading} onClick={() => handleApprove(r._id)} />
                            <button onClick={() => { setSelected(r._id); setShowReject(true); }}
                              ...
                              style={{ background:'linear-gradient(135deg,#dc2626,#b91c1c)', minWidth:145 }}>
                              ❌ Reject
                            </button>
                          </div>
              ...
              {ssModal.status==='pending' && (
                <div className="p-4 border-t border-purple-500/20 flex gap-3">
                  <div className="flex-1">
                    <ApproveBtn id={ssModal._id} actionLoading={actionLoading} onClick={() => handleApprove(ssModal._id)} />
                  </div>
