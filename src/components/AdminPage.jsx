...
export default function AdminPage() {
  ...
  const fetchSS = async (id) => {
    ...
    } catch { alert('Failed to load screenshot.'); }
  };
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) { alert('✅ Approved! Email sent.'); fetchData(tab); setSsModal(null); }
  ...
  const downloadExcel = () => {
    const rows = data.map((r,i) => ({
      ...
      'Status': r.status, 'Date': new Date(r.createdAt).toLocaleString(),
    }));
  ...
  const filtered = data.filter(r =>
    [r.name, r.email, r.teamName, r.collegeName].join(' ').toLowerCase().includes(search.toLowerCase())
  );

  const tabCounts = { pending: '⏳', approved: '✅', rejected: '❌' };
        ...
        <div className="flex gap-2 mb-5 flex-wrap">
          {['pending','approved','rejected','all'].map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {tabCounts[t] || '📋'} {t.charAt(0).toUpperCase()+t.slice(1)}
            </button>
          ))}
        </div>

        {/* Search */}
        <input type="text" placeholder="Search by name, email, team..." value={search}
          onChange={e => setSearch(e.target.value)}
          className="w-full rounded-xl px-5 py-3 text-sm text-white placeholder-gray-600 outline-none mb-5"
          style={{ background:'rgba(139,92,246,0.06)', border:'1px solid rgba(139,92,246,0.2)' }} />

        {loading && <div className="flex justify-center py-20"><motion.div className="w-10 h-10 rounded-full border-2 border-purple-500 border-t-transparent" animate={{ rotate:360 }} transition={{ duration:0.8, repeat:Infinity, ease:'linear' }} /></div>}
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border:'1px solid rgba(139,92,246,0.2)' }}>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={8} className="text-center text-gray-500 py-16">No registrations found.</td></tr>
                  ) : filtered.map((r,i) => (
                    <motion.tr key={r._id}
                      ...
                      <td className="px-4 py-3 text-cyan-400 whitespace-nowrap text-xs">{r.email}</td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        ...
                        {r.status !== 'pending' && <span className="text-gray-600 text-xs">—</span>}
                      </td>
                    </motion.tr>
                  ))}
                </tbody>
              ...
              {ssModal.status === 'pending' && (
                <div className="p-4 border-t border-purple-500/20 flex gap-3">
                  <button onClick={() => handleApprove(ssModal._id)} disabled={!!actionLoading}
                    ...
                    style={{ background:'linear-gradient(135deg,#16a34a,#15803d)' }}>
                    {actionLoading === ssModal._id+'_approve' ? 'Processing...' : '✅ Approve & Send Email'}
                  </button>
